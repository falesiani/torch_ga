...
class TensorToGeometric(GeometricAlgebraLayer):
    ...
    def __init__(self, algebra: GeometricAlgebra, blade_indices: List[int],
        ...
        self.blade_indices = torch.tensor(
...
class GeometricProductDense(GeometricAlgebraLayer):
    ...
    def build(self, input_shape: list):
        ...
        self.kernel = nn.Parameter(1./np.prod(shape_kernel)*torch.randn(shape_kernel), dtype=torch.float)
        if self.use_bias:
            shape_bias = [self.units, self.blade_indices_bias.shape[0]]
            self.bias = nn.Parameter(1./np.prod(shape_bias)*torch.randn(shape_bias), dtype=torch.float)
        else:
            self.bias = None
...
class GeometricProductElementwise(GeometricAlgebraLayer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.blade_indices_kernel = torch.tensor(
            blade_indices_kernel, dtype=torch.int64)
        if use_bias:
            self.blade_indices_bias = torch.tensor(
                blade_indices_bias, dtype=torch.int64)
